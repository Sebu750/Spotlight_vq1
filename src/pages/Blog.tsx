...
import { Search, ArrowRight, Bookmark, Clock, Share2, Tag, Mail } from 'lucide-react';
    ...
    <div className="pt-24 min-h-screen">
      ...
      <section className="px-6 pb-20 border-b border-white/10">
        <div className="max-w-7xl mx-auto">
          <h1 className="text-6xl md:text-[10rem] font-black tracking-tighter leading-[0.8] mb-12 uppercase italic relative">
            THE <br /><span className="text-white ml-0 md:ml-40">CHRONICLES</span>
            <span className="absolute -top-10 -right-10 text-[20vw] text-accent opacity-5 font-sans italic select-none">READ.</span>
          </h1>
          
          <div className="flex flex-wrap lg:row items-center justify-between gap-8 mt-20">
            <div className="flex flex-wrap gap-4">
              {categories.map((cat) => (
                <button
                  key={cat}
                  onClick={() => setActiveCategory(cat)}
                  className={`px-6 py-2 font-sans font-bold uppercase text-[10px] tracking-widest transition-all ${
      ...
      <section className="px-6 py-20 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
          ...
          <div className="lg:col-span-8 space-y-24">
            {filteredArticles.map((article, idx) => (
              <motion.article 
                ...
                viewport={{ once: true }}
                className="group flex flex-col md:row gap-10 items-start"
              >
          ...
          {/* Sidebar */}
          <aside className="lg:col-span-4 space-y-16 lg:sticky lg:top-32 hidden lg:block">
            {/* Newsletter */}
            <div className="p-8 bg-accent border border-white/10 shadow-2xl relative overflow-hidden">
              <div className="absolute top-0 right-0 opacity-10 -mr-10 -mt-10">
                <Mail size={200} className="text-white" />
              </div>
              <h4 className="text-3xl font-black text-white mb-4 italic uppercase tracking-tighter">THE FUTURE IN YOUR INBOX.</h4>
              <p className="text-white/80 font-serif italic mb-8">Join 15,000+ industry shakers. No fluff, just signal.</p>
              <div className="space-y-4">
                <input 
                  type="email" 
                  placeholder="EMAIL@FUTURE.COM" 
                  className="w-full bg-white/20 border border-white/20 px-6 py-4 outline-none focus:bg-white focus:text-dark transition-all font-sans text-xs tracking-widest placeholder:text-white/50"
                />
                <button className="w-full bg-white text-accent py-4 font-sans font-black uppercase text-sm tracking-widest hover:scale-[1.02] transition-all">
                  Subscribe
                </button>
              </div>
            </div>
            ...
            <div className="aspect-square bg-white relative overflow-hidden group">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center p-12 text-center">
                <h3 className="text-white text-4xl font-black uppercase tracking-tighter mb-8 group-hover:scale-110 transition-transform">Your Work. Our Stage.</h3>
                <Link to="/apply" className="bg-accent text-white px-8 py-4 font-sans font-black uppercase text-xs tracking-[0.2em] shadow-2xl">
                  Apply Now
                </Link>
              </div>
            </div>
      ...
      <section className="lg:hidden px-6 py-20 bg-accent text-center">
        ...
        <div className="max-w-md mx-auto space-y-4">
          <input 
            type="email" 
            placeholder="EMAIL@FUTURE.COM" 
            className="w-full bg-white/20 border border-white/20 px-6 py-4 outline-none focus:bg-white focus:text-dark transition-all font-sans text-xs tracking-widest"
          />
          <button className="w-full bg-white text-accent py-4 font-sans font-black uppercase text-sm tracking-widest shadow-2xl">
            Subscribe
          </button>
        </div>
      </section>
